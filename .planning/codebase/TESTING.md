...
- CI runs both stacks through `.github/workflows/nightly-sweep.yml`.
...
**Run Commands:**
```bash
.\.venv\Scripts\python.exe -m pytest -q                                     # Backend default suite; `pytest.ini` deselects `nightly`
...
**Requirements:** No coverage threshold, coverage script, or coverage config file was detected. Quality gates rely on targeted suites plus CI jobs in `.github/workflows/nightly-sweep.yml`.
...
- Backend pure logic tests: `tests/backend/test_allocator.py`, `tests/backend/test_prereq_parser.py`, `tests/backend/test_semester_recommender.py`, `tests/backend/test_server_data_reload.py`, and `tests/backend/test_nightly_analyze.py`.
