...
| Tests | pytest (backend), Next.js lint/build validation (frontend) |
...
| Module | Responsibility |
|--------|---------------|
| `frontend/src/app/layout.tsx` | Global app shell, fonts, provider wiring |
| `frontend/src/context/AppContext.tsx` | Shared planner session state and reducer dispatch |
| `frontend/src/lib/api.ts` | API calls (`loadCourses`, `loadPrograms`, `postRecommend`, `postCanTake`) |
| `frontend/src/components/planner/PlannerLayout.tsx` | Main planner workspace composition |
| `frontend/src/hooks/useRecommendations.ts` | Recommendation request payload and fetch lifecycle |
| `frontend/src/hooks/useSession.ts` | localStorage session restore/persist for planner |
...
|-------|---------|----------------|
| Backend | `python -m pytest tests/backend -q` | 366 |
| Frontend | `cd frontend && npm run lint && npm run build` | Build/Lint gates |

### 9.3 CI Pipeline

`.github/workflows/validate.yml` runs on every push/PR to main:
1. `python scripts/validate_track.py --all`
2. `python -m pytest tests/backend -q`
