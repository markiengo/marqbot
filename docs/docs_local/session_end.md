...
## 4) Run checks
### Backend changes
- Always start with the smallest useful check:
  - run the closest focused backend test file(s) for the code you changed
- Run the standard backend suite only when the change is broad enough to justify it:
  - `python -m pytest -q`
- Broad enough usually means:
  - shared backend modules like `server.py`, `semester_recommender.py`, `eligibility.py`, `allocator.py`, `validators.py`
  - request/response contract changes
  - data model or loader changes
  - changes that affect multiple majors, tracks, or semesters
- If planner or recommendation logic changed:
  - also run `python -m pytest tests/backend/test_dead_end_fast.py -q`
- Do not run the nightly sweep by default.
- Do not run the entire backend suite if a focused test gives the needed proof and the change is narrow.

### Frontend changes
- Start with the closest focused frontend test.
- Run `cd frontend && npm run test` only if the change touches shared UI/state/helpers or multiple screens.
- Run `cd frontend && npm run lint` and `cd frontend && npm run build` when frontend code changed in a pushable way.

### Data changes
- Run the focused backend tests that exercise the changed data path.
- Run `python -m pytest -q` if the data change affects runtime loading, recommendation behavior, or shared program logic.
