...
- Are any tests now redundant, misleading, or guarding behavior that no longer exists?
- If a test looks removable, what exact remaining test still protects that regression?
...
6. **Audit the tests too**
   - look for dead tests, duplicate cases, misleading names, and over-broad slow checks
   - do not remove a test unless you can point to the surviving coverage
   - if a test should be removed, say whether to delete it now or only after replacement coverage lands

## 9) MarqBot validation shortcuts

### Backend logic change
- closest focused backend test file first
- `.\.venv\Scripts\python.exe -m pytest -q` for the standard backend suite when the change is shared or cross-cutting
...
- closest focused frontend test first
- `cd frontend && npm run test` when shared frontend behavior changed
...
- **Test cleanup candidates**
  - only include when there is strong evidence a test is redundant, stale, or mis-scoped
  - name the replacement coverage or state that replacement is required before deletion
- **Residual risks**
