...
- `course_equivalencies.csv` handles equivalence expansion for completed/in-progress credit checks, prereq satisfaction, and dedup filtering. It no longer expands bucket mappings — `master_bucket_courses.csv` is the single source of truth for course-to-bucket mapping.
