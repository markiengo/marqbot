  ...
  return (
    <div className="h-full min-h-0 rounded-2xl border border-border-subtle bg-[#0b2143]/70 p-3 flex flex-col gap-1.5">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {hasData && (
          <button
            ...
            className="text-[10px] font-semibold text-ink-primary bg-navy/70 border border-border-medium rounded-lg px-2 py-0.5 hover:bg-navy-light transition-colors cursor-pointer"
          >
            View Full Progress
          </button>
        )}
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-2.5 pb-1">
        <div className="grid grid-cols-3 gap-2 flex-[1.1] min-h-0">
        ...
        <div className="rounded-xl border border-border-subtle bg-surface-card/40 p-3 text-center flex-[0.9] min-h-[92px] max-h-[140px] flex flex-col justify-center">
          ...
          <p className="text-ink-muted mt-0.5 text-[10px]">Your Status</p>
        </div>
      </div>
    </div>
  );
...
function KpiTile({
  ...
  return (
    <div className="h-full rounded-lg border border-border-subtle bg-surface-card/40 p-2 text-center min-h-0 flex flex-col items-center justify-center">
      <div className={`text-xl font-bold font-[family-name:var(--font-sora)] leading-none ${valueClass}`}>
        {value}
      </div>
      <div className="text-[10px] text-ink-secondary mt-1 leading-tight">{label}</div>
    </div>
  );
}
